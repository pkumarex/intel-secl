 ...
 */
 ...
 func createPCRManifest(hostTpmAttestationReport *vim25Types.HostTpmAttestationReport) (types.PcrManifest, error) {
	 ...
	 pcrManifest.PcrEventLogMapLinux = pcrEventLogMap
	 return pcrManifest, nil
 }
 
 func getPcrEventLog(hostTpmEventLogEntry []vim25Types.HostTpmEventLogEntry, eventLogMap types.PcrEventLogMapFC) (types.PcrEventLogMapFC, error) {
	 ...
	 for _, eventLogEntry := range hostTpmEventLogEntry {
		 ...
		 if err != nil {
			 return types.PcrEventLogMapFC{}, errors.Wrap(err, "vmware_host_connector:getPcrEventLog() Error "+
		 ...
		 if err != nil {
			 return types.PcrEventLogMapFC{}, err
		 }
