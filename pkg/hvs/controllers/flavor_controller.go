					...
					"event_log": [
						{
							...
							"value": "6610dde8da53671165c3065359bab14f5a9fc1d9f0c4493732546c30ac572ef4",
							"label": "ISecL_Default_Workload_Flavor_v2.1-6ddd0a43-736c-4369-abfd-9a1e22a9b735",
							"info": {
								"ComponentName": "ISecL_Default_Workload_Flavor_v2.1-6ddd0a43-736c-4369-abfd-9a1e22a9b735",
								"EventName": "OpenSource.EventName"
							}
						},
						{
							"digest_type": "com.intel.mtwilson.core.common.model.MeasurementSha256",
							"value": "f4b9dde27d77788d82466005c3c2b467e0bcaed6a4852cec31291835a99ee54a",
							"label": "ISecL_Default_Application_Flavor_v2.1_TPM2.0-a2c616e0-9ace-4dab-adbc-1d10b8cbeb4a",
							"info": {
								"ComponentName": "ISecL_Default_Application_Flavor_v2.1_TPM2.0-a2c616e0-9ace-4dab-adbc-1d10b8cbeb4a",
								"EventName": "OpenSource.EventName"
							}
						}
					],
...
func (fcon *FlavorController) createFlavors(flavorReq dm.FlavorCreateRequest) ([]hvs.SignedFlavor, error) {
	...
	if flavorReq.ConnectionString != "" {
		...
		// Here we will know the host's information, That we usually mention in template conditions
		// Access hostManifest , get values from it , hostManifest.HostInfo compare these values with the templates stored
		// in DB and retrieve possbile templates .
		// Certainly Default template is applicable for all the scenarios, So if one or more templates has the same conditions
		// Then What template we need to choose?
		flavorTemplate, err := fcon.getFlavorTemplate(*hostManifest)
		if err != nil {
			return nil, errors.Wrap(err, "Failed to match flavor templates for required hostmanifest")
		}
		defaultLog.Debugf("Matched template  %s", flavorTemplate)
...
func (fcon *FlavorController) getFlavorTemplate(hostManifest hcType.HostManifest) ([]hvs.FlavorTemplate, error) {
	defaultLog.Trace("controllers/flavor_controller:getFlavorTemplate() Entering")
	defer defaultLog.Trace("controllers/flavor_controller:getFlavorTemplate() Leaving")
	var filteredTemplates []hvs.FlavorTemplate

	flavorTemplates, err := fcon.FTStore.Search(false)
	if err != nil {
		return nil, errors.Wrap(err, "Could not retrieve all flavor templates from Database")
	}

	hostManifestJson, err := jsonquery.Parse(strings.NewReader(string(hostManifestString))) // tempCode
	if err != nil {
		defaultLog.Debug("Error in parsing the host manifest")
	}

	for _, flavorTemplate := range flavorTemplates {
		conditionEval := true
		defaultLog.Debug("checkpoint A")
		for _, condition := range flavorTemplate.Condition {
			defaultLog.Debug("Condition -> ", condition)
			expectedData, err := jsonquery.Query(hostManifestJson, condition)
			if err != nil {
				defaultLog.Debug("Error in condition json query -> ", err)
			}
			if expectedData == nil {
				conditionEval = false
				defaultLog.Debug("**************False" + condition)
				break
			} else {
				defaultLog.Debug("Excepted data -> ", expectedData.Data)
				defaultLog.Debug("**************" + condition)
			}
		}
		if conditionEval == true {
			defaultLog.Debug("**************Eval=true")
			filteredTemplates = append(filteredTemplates, flavorTemplate)
		}
	}
	defaultLog.Debug("Filtered Templates -> ", filteredTemplates)
