...
func processJsonFile(manifestFilepath string, flavorTemplates []string) (hcType.HostManifest, []hvs.FlavorTemplate, error) {
	...
	if err != nil {
		return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Could not load host manifest file")
	}

	if len(hostManifestJSON) == 0 {
		return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Empty hostmanifest file given, unable to proceed further")
	}

	// Validate the format
	if !json.Valid(hostManifestJSON) {
		return hcType.HostManifest{}, nil, errors.New("flavor_gen:processJsonFile() Hostmanifest file is not a valid json")
	}
	err = json.Unmarshal(hostManifestJSON, &hostManifest)
	if err != nil {
		fmt.Errorf("Could not unmarshal host manifest json %s", err)
		return hcType.HostManifest{}, nil, errors.New("flavor_gen:processJsonFile() Could not unmarshal host manifest json")
	}

	manifest, err := jsonquery.Parse(bytes.NewReader(hostManifestJSON))
	if err != nil {
		fmt.Errorf("Could not parse host manifest json %s", err)
		return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Could not parse host manifest json")
	}
	...
	for _, template := range flavorTemplates {
		...
		if err != nil {
			return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Could not load flavor template file")
		}

		if len(flavorJSON) == 0 {
			return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Empty flavor template file is given, unable to proceed further")
		}

		// Validate the format
		if !json.Valid(flavorJSON) {
			return hcType.HostManifest{}, nil, errors.New("flavor_gen:processJsonFile() Given flavor template file is not a valid json")
		}

		err = json.Unmarshal(flavorJSON, &flavorTemplate)
		if err != nil {
			return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Could not unmarshal flavor template json")
		}

		if flavorTemplate.ID == uuid.Nil {
			flavorTemplate.ID, err = uuid.NewRandom()
			if err != nil {
				return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Failed to generate UUID for flavor template")
			}
		}
		...
		for _, condition := range flavorTemplate.Condition {
			...
			if err != nil {
				return hcType.HostManifest{}, nil, errors.Wrap(err, "flavor_gen:processJsonFile() Failed to query search condition with hostmanifest")
			}
			if expectedData == nil {
				conditionEval = true
				break
	...
	if len(flavors) == 0 {
		return hcType.HostManifest{}, nil, errors.New("flavor_gen:processJsonFile() Condition does not matches with manifest file")
	}
...
func validateTemplate(templateFilePath string) (string, error) {
	...
	definitionsSchema := gojsonschema.NewStringLoader("/etc/hvs/schema/common.schema.json")

	flvrTemplateSchema := gojsonschema.NewStringLoader("/etc/hvs/schema/flavor-template.json")
